...
void Dictionary::add(const std::string& w) {
  int32_t h = find(w);
  std::cerr<<"hash index: "<<h<<std::endl;
  ntokens_++;
  if (word2int_[h] == -1) {
    entry e;
    e.word = w;
...
void Dictionary::computeSubwords(
  ...
  for (size_t i = 0; i < word.size(); i++) {
    ...
    /*
      0: single byte
      10: continution byte of a multibyte character
      110: the first byte of a multibyte character, 11 refers that there are two bytes of this character including the first one
      the number of leading 1's indicates how many bytes that combine the whole multibyte character together
    */
    if ((word[i] & 0xC0) == 0x80) {
      continue;
    }
