...
void FastText::loadModel4Java(const char* filename) {
  std::string path(filename);
  std::ifstream ifs(path, std::ifstream::binary);
  if (!ifs.is_open()) {
    throw std::invalid_argument(path + " cannot be opened for loading!");
  }
  if (!checkModel(ifs)) {
    throw std::invalid_argument(path + " has wrong file format!");
  }
  loadModel(ifs);
  ifs.close();
}
...
/*
void FastText::predictSentence(
  std::string& sentence,
  std::pair<real, std::string>& prediction,
  int32_t topK,
  real threshold) const
{
  std::vector<int32_t> words, labels;
